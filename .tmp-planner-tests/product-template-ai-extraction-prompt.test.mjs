...
import { buildProductTemplateSetupPlanningPrompt, extractionPromptFocuses, getProductTemplateAiExtractionPrompt } from "./product-template-ai-extraction-prompt.js";
...
test("focused prompts retain the v1 contract, price safety, source fidelity, and relevant target", () => {
    ...
    const targets = { full: "Full Product / Complete Extraction", base_model: "pricing.baseModelRows", workstation: "pricing.workstationRows", category_matrix: "pricing.priceMatrices", modular: "pricing.modularGroups", accessories: "optionGroups", product_details: "Product Details / Specifications", materials: "materialSuggestions" };
    ...
    ["full", "base_model", "workstation", "category_matrix", "modular"].forEach((focus) => assert.ok(getProductTemplateAiExtractionPrompt(focus).includes("Also extract any clearly related accessories")));
