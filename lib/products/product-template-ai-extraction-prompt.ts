...
The words "workstation", "bench", "cluster", and "operative" do not automatically mean pricing.workstationRows. First identify the manufacturer's actual commercial pricing architecture for that family, evaluating in this order: (1) a complete direct-priced SKU with one direct authoritative price, which belongs in pricing.baseModelRows even when the manufacturer calls it a workstation, bench, cluster, or operative desk; (2) simple supported Workstation/bench pricing that fits the current narrow pricing.workstationRows model (authoritative complete row price, optional supported additional/cluster price, no free-form starter/add-on composition); (3) genuine manufacturer-proven row-by-category or finish-price-class pricing, which belongs in pricing.priceMatrices; (4) source-proven component/module composition such as starter/add-on or structure-plus-top-plus-finish pricing, which belongs in pricing.modularGroups. Use the structure that preserves manufacturer truth with the least unnecessary complexity, and never move a direct-priced workstation/bench SKU into pricing.workstationRows merely because the source uses one of those words.

APPLICABILITY TARGETS FOLLOW ACTUAL PRICING ROUTING
Choose target.kind only AFTER the authoritative row has been emitted into its final pricing structure; never infer target.kind from product words such as workstation, bench, or module. If the row is in pricing.baseModelRows, use target.kind "base_model" and group_id "${LEGACY_BASE_MODEL_GROUP_ID}". If it is in pricing.workstationRows, use target.kind "workstation" and group_id "${LEGACY_WORKSTATION_GROUP_ID}". If it is in pricing.priceMatrices[n].rows, use target.kind "price_matrix" and that pricing.priceMatrices[n].id. If it is in either pricing.modularGroups[n].matrix.rows or pricing.modularGroups[n].directRows, use target.kind "modular" and that pricing.modularGroups[n].id. In every case row_id is the exact already-emitted row.id, never a supplier code. Never point a Base/Model, Matrix, or Modular row at a workstation target.
...
Evaluate in this order: (1) a complete direct-priced SKU with its own supplier code and one authoritative price routes to pricing.baseModelRows, even when the manufacturer calls it a workstation, bench, DX/SX unit, or complete desk-plus-service-unit SKU; (2) a simple, complete-price workstation/bench family that fits the current narrow workstationRows model (authoritative complete row price, optional supported additional/cluster price, no free-form composition) routes to pricing.workstationRows; (3) genuine manufacturer-proven row-by-category or finish-price-class pricing routes to pricing.priceMatrices; (4) source-proven component/module composition, including starter/add-on systems, routes to pricing.modularGroups. Use the structure that preserves manufacturer truth with the least unnecessary complexity.
...
Do not blindly attach every accessory shown on a technical accessories page merely because it appears there. Preserve a clearly compatible item as an option only when the source proves its applicability to the selected family/rows. When applicability is uncertain, preserve the item as a linkedFamilySuggestion or add an extractionWarning describing the uncertainty; never silently drop a relevant supplied accessory page without warning, and never invent applicability to force an item into optionGroups.
...
If the manufacturer gives separate priced codes for with/without electrification, E/non-E, LH/RH, or with/without cable access (for example TE160 and TE160E), preserve those as separate authoritative priced rows. Do not automatically convert TE160E into TE160 plus an electrification option when the manufacturer prices both as distinct SKU rows.
...
ProductTemplateDraft v1 baseModelRows is flat. If the source contains multiple meaningful direct-price model families, do not flatten away those family boundaries and do not create a fake one-column priceMatrix merely to preserve them: a one-column "Price" matrix remains a forbidden fake Matrix no matter how many families exist. Instead keep every row in pricing.baseModelRows, make each row's family identity explicit in its own label/displayName/specification (for example a family name/prefix or a short family note), and add an extractionWarning naming the distinct families present so Review & Route can split them into separate Base / Model Pricing groups when ProjectWorkflow supports it. Never restore a fake Matrix to represent that grouping.
...
- applicability: an array of rules, one per exact applicable row; each rule needs a COMPLETE target: { kind: "base_model" | "price_matrix" | "modular" | "workstation", group_id, row_id } naming the precise Base/Model, Price Matrix, Modular, or Workstation row this rule targets. Never leave a row-specific rule without a target, never omit group_id even when it looks implied by kind, and never create one unscoped/global rule when the source names specific rows.
...
- scale_with_target_quantity: optional boolean, only for a Modular target with fixed_quantity and selection other than exactly_one — use "at_least_one" for this shape, never "exactly_one" (the runtime rejects that combination). When true, multiply fixed_quantity by the selected target-row quantity. For ART.058 at EUR 69, use conditionalConfiguration.selection "at_least_one" with fixed_quantity: 2 and scale_with_target_quantity: true: one selected starter needs 2, starter plus one intermediate needs 4, and starter plus two intermediates needs 6; never create an EUR 138 synthetic item.
...
Choose target.kind and group_id only after routing the authoritative row into its final pricing structure. row_id always equals the exact stable id already assigned to that row/item in that structure (for example "oxi-q-ws-dx") — never a supplierCodes entry such as "111 623", and never invented from a code, label, or family name. For target.kind "price_matrix" or "modular", group_id is the exact pricing.priceMatrices[].id or pricing.modularGroups[].id that contains that row. pricing.baseModelRows and pricing.workstationRows are flat arrays with no group wrapper in this draft; use group_id: "${LEGACY_BASE_MODEL_GROUP_ID}" only for an actual base_model target and group_id: "${LEGACY_WORKSTATION_GROUP_ID}" only for an actual workstation target. Use these exact fixed strings verbatim; do not invent a different group_id, leave it blank, or derive one from the row/family name.
...
15. No undocumented ProductTemplateDraft fields were added; selectionFamily is used only according to the documented Direct Modular Selection Family contract.
