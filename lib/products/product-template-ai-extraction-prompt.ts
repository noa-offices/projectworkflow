...
When manufacturer evidence proves that a separately priced furniture item acts as structural support/base equipment for the target commercial system, route it as an optionGroups item with role "structural_support" even if it has a direct price, dimensions, or supplier code. This takes precedence over the ordinary direct-priced primary SKU -> Base/Model fallback. Do not put it in pricing.baseModelRows, pricing.workstationRows, pricing.priceMatrices, or pricing.modularGroups unless the source proves it is itself the primary independently selectable product family. Apply this only when explicit wording or construction/completion logic proves the target system integrates with, fixes to, rests on, or is completed by that support; never infer it from cabinet, pedestal, storage, service unit, support, return, bridge, nearby placement, visual proximity, or similar names. If the structural-support item explicitly requires another separately priced item, keep both in optionGroups: the dependent companion uses target.kind "option_item" with the exact structural-support group/item IDs and source-proven fixed_quantity. Do not target the support as base_model merely to express the dependency.
...
When manufacturer evidence proves that a separately priced service cabinet, storage unit, or support item acts as structural support/base equipment for the selected system, preserve that option item with role "structural_support" rather than presenting it as an ordinary optional accessory. If another supplied item is explicitly mandatory because that structural-support item is selected, encode that dependency using target.kind "option_item". Do NOT infer structural support from cabinet/storage/support words alone, desk filtering, or desk-to-cabinet quantity unless separately proven.
...
Verify that a source-proven separately priced structural support item was not rendered as an ordinary optional accessory; any explicitly required support-triggered companion uses exact option_item option-group/item IDs; no relationship was invented from naming or proximity; and no unsupported deeper dependency graph was fabricated.
