...
export function applicabilityTargetChoice(
  ...
  const displayName = row.display_name?.trim() || row.variant_name?.trim() || row.label?.trim() || row.default_dimension?.trim() || row.dimension?.trim() || "Unnamed model";
  const candidateCode = row.supplier_price_list_code?.trim() || row.base_supplier_price_list_code?.trim() || row.variant_name?.trim() || "";
...
export function applicabilityTargetChoices(
  ...
  workstationGroups: PricingGroup[] = [],
  ...
  return [
    ...
    ...workstationGroups.flatMap((group) => (group.items ?? []).flatMap((row) => applicabilityTargetChoice("workstation", group, row) ?? [])),
  ];
}
