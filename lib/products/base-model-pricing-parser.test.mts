...
test("server parser accepts legacy rows and preserves IDs, null, zero, and exact values", () => {
  const parsed = parseBaseModelPricingJson(JSON.stringify([row, { ...row, id: "zero", price: 0 }, { ...row, id: "exact", price: 123.45 }])) as typeof row[];
...
test("legacy UI normalization saves one explicit group and preserves row identity", () => {
  ...
  assert.equal((saved[0] as { pricing_type: string }).pricing_type, BASE_MODEL_GROUP_PRICING_TYPE);
  assert.equal((saved[0] as { items: Array<{ id: string }> }).items[0].id, "model-a");
});
