...
export function productTemplateFormSmartWorkspace(snapshot: FormSnapshot): { draft: ProductTemplateDraft; plan: SmartSetupReviewRoutingPlan; subgroups: SmartReviewedPricingSubgroups } {
  ...
  const baseRows: ProductTemplateDraftBaseModelRow[] = baseGroups.flatMap((group) => group.items.map((item) => ({
    ...row(item),
    ...(group.isSyntheticLegacyGroup ? {} : { groupId: group.id, ...(text(group.group_name) ? { groupLabel: text(group.group_name)! } : {}) }),
