...
import {
  ...
  hasExplicitBaseModelPricingGroupStructure,
  hasMeaningfulBaseModelPricingData,
...
function group(id: string, name: string, items: typeof legacyRows, active = true, sortOrder = 0) {
...
test("duplicate ids and reserved synthetic-id conflicts produce deterministic validation errors", () => {
  assert.throws(
    () => baseModelPricingGroups([group("same", "One", []), group("same", "Two", [])]),
    (error) => error instanceof BaseModelPricingContractError && error.issues.some((issue) => issue.code === "duplicate_group_id"),
  );
  assert.doesNotThrow(() => baseModelPricingGroups([group(LEGACY_BASE_MODEL_GROUP_ID, "Saved legacy", [])]));
  assert.throws(
    () => baseModelPricingGroups([legacyRows[0], group(LEGACY_BASE_MODEL_GROUP_ID, "Conflict", [])]),
    /conflicts with the synthesized legacy group/,
  );
});
...
test("invalid roots, malformed groups and rows, and unsupported discriminators report issues safely", () => {
  ...
  assert.throws(() => baseModelPricingGroups([group("bad-row", "Bad row", [null as never])]), /invalid row/);
  assert.throws(() => baseModelPricingGroups([group("nested-type", "Nested", [{ id: "nested", pricing_type: "other_group" } as never])]), /invalid row/);
...
test("ProductTemplateDraft v1 remains flat and has no Base/Model groups key", () => {
  type HasBaseModelGroups = "baseModelGroups" extends keyof ProductTemplateDraft["pricing"] ? true : false;
  const hasBaseModelGroups: HasBaseModelGroups = false;
  const baseModelRowsKey: Extract<keyof ProductTemplateDraft["pricing"], "baseModelRows"> = "baseModelRows";
