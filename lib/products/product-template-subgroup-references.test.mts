...
test("accessory subgroup identity uses the same canonical group/subgroup key", () => { const accessoryPricing = [{ id: "accessory-g", items: [{ id: "item" }], subgroups: [{ id: "size", subgroup_name: "Size", sort_order: 0, is_active: true, row_ids: ["item"] }] }]; const allPricing = { accessoryPricing }; assert.deepEqual(resolveProductTemplateSubgroupIdentity(allPricing, "accessory", "size", "accessory-g"), { groupId: "accessory-g", subgroupId: "size" }); assert.ok(persistedProductTemplateSubgroupKeys(allPricing).has(baseModelSubgroupReferenceKey("accessory", "accessory-g", "size"))); });
