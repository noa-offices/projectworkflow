...
import { addBaseModelPricingRow, createBaseModelPricingGroup, removeBaseModelPricingGroup, removeBaseModelPricingRow, replaceWholeTemplateBaseModelRows, replaceWholeTemplateBaseModelPricing, shouldApplyBaseModelReplacement, updateBaseModelPricingGroup, } from "./base-model-pricing-ui-state.js";
test("group and row mutations preserve identity and isolate siblings", () => {
    const first = { ...createBaseModelPricingGroup("group-a", 0, "Executive"), items: [{ id: "row-a" }] };
    const second = { ...createBaseModelPricingGroup("group-b", 1, "Meeting"), items: [{ id: "row-b" }] };
...
test("new group IDs remain stable and empty groups preserve null, zero, and exact prices", () => {
    const group = createBaseModelPricingGroup("stable-id", 0);
    ...
    assert.deepEqual(createBaseModelPricingGroup("empty", 1).items, []);
...
test("whole-template replacement preserves a sole destination and resolves ambiguity to one group", () => {
    const sole = { ...createBaseModelPricingGroup("current", 0), items: [{ id: "manual" }] };
    assert.equal(replaceWholeTemplateBaseModelRows([sole], [{ id: "imported" }], "unused")[0].id, "current");
    const replaced = replaceWholeTemplateBaseModelRows([sole, createBaseModelPricingGroup("other", 1)], [{ id: "imported" }], "import-group");
...
test("grouped Smart Setup replacements preserve flat-row behavior and append source families", () => {
    const existing = [createBaseModelPricingGroup("existing", 0, "Existing")];
    const converted = [{ ...createBaseModelPricingGroup("matrix", 1, "Executive Desks"), items: [{ id: "matrix-row" }] }];
